...
def test_configuration():
    ...
    if os.getenv('ANTHROPIC_API_KEY') and os.getenv('ANTHROPIC_API_KEY') != 'your_api_key_here':
        print("   [PASS] API key configured in environment")
        return True
    ...
    if env_file.exists():
        ...
        with open(env_file) as f:
            ...
            if 'ANTHROPIC_API_KEY' in content and 'your_api_key_here' not in content:
                print("   [PASS] API key configured in .env")
                return True
            else:
                print("   [WARN] API key not configured (edit .env)")
                return False
    else:
        print("   [FAIL] .env file not found and ANTHROPIC_API_KEY not in environment")
...
def test_workspace():
    ...
    try:
        ...
        if workspace_path.exists():
            print(f"   [PASS] Workspace exists: {workspace_path}")
            return True
        else:
            print(f"   [WARN] Workspace doesn't exist: {workspace_path}")
...
def test_imports():
    ...
    print("\n[Modules] Testing Local Helper modules...")
    ...
    try:
        ...
        print("   [PASS] file_operations module")

        from src.task_executor import TaskExecutor
        print("   [PASS] task_executor module")

        from src.document_processor import DocumentProcessor
        print("   [PASS] document_processor module")

        from src.agent import LocalHelperAgent
        print("   [PASS] agent module")

        from src.cli import CLI
        print("   [PASS] cli module")
